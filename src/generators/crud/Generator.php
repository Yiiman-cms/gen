<?php
	...
	namespace YiiMan\gen\generators\crud;
	 ...
	 * @property string                   $controllerID     The controllers ID (without the module ID prefix). This
	 ...
	 * @since  2.0
	 */
	class Generator extends \YiiMan\gen\Generator {
		public $CrudName;
		public $modelClass;
		...
		public function getDescription() {
			return 'این موتور خودساز کنترلر،ویو و مدل جست و جو را برای آدرس های تعریف شده ایجاد میکند.';
		}
		...
		public function rules() {
			return array_merge(
				...
				[
					[
						[ 'controllerClass' , 'modelClass' , 'searchModelClass' , 'baseControllerClass' , 'CrudName' ] ,
						'filter' ,
						'filter' => 'trim'
					] ,
					[
						[ 'modelClass' , 'controllerClass' , 'baseControllerClass' , 'indexWidgetType' , 'CrudName' ] ,
						'required'
					] ,
					[
						[ 'searchModelClass' ] ,
					...
					[
						[ 'baseControllerClass' ] ,
						'validateClass' ,
						'params' => [ 'extends' => Controller::className() ]
					] ,
					[
						[ 'controllerClass' ] ,
						'match' ,
		...
		public function hints() {
			return array_merge(
				parent::hints() ,
				[
					'CrudName'            => 'نام بخش در واقع همان تیتر موضوعی میباشد که به کاربر نمایش داده میشود.' ,
		...
		public function generate() {
			$controllerFile = Yii::getAlias(
				'@' . str_replace( '\\' , '/' , ltrim( $this->controllerClass , '\\' ) ) . '.php'
			);
			...
			if ( ! empty( $this->searchModelClass ) ) {
				$searchModel = Yii::getAlias(
					'@' . str_replace( '\\' , '/' , ltrim( $this->searchModelClass , '\\' ) . '.php' )
				);
		...
		public function getViewPath() {
			...
			if ( ! empty( $this->viewPath ) ) {
				$pieces         = preg_split( '/(?=[A-Z])/' , $this->viewPath );
				$this->viewPath = '';
		...
		/**
		 * Generates id of Model
		 * @return string
		 */
		public function generateUrlModelID() {
			/* @var $class ActiveRecord */
			$class = $this->modelClass;
			$pks   = $class::primaryKey();
			if ( count( $pks ) === 1 ) {
		...
		public function generateActionParamComments() {
			...
			if ( ( $table = $this->getTableSchema() ) === false ) {
				$params = [];
				foreach ( $pks as $pk ) {
					$params[] = '@param ' . ( strtolower(
						                          substr( $pk , - 2 )
					                          ) === 'id' ? 'integer' : 'string' ) . ' $' . $pk;
				}
